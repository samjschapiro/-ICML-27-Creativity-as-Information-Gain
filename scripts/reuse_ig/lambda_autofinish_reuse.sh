#!/bin/bash
# Unattended finisher for the reuse items: wait for ALL_DONE, fetch, verify every GPU reader's file
# has as many lines as items.jsonl, terminate the instance. Hard cap so an idle box cannot bill.
set -u
IP="${1:?ip}"; INST="${2:?instance id}"; EXPECT=$(wc -l < data/reuse_ig/items/items.jsonl); MAX_WAIT_MIN=180
...
while true; do
  status=$($SSH 'tail -1 ~/reuse_ig/logs/run_all.log 2>/dev/null' 2>/dev/null || echo "ssh-failed")
  elapsed=$(( ($(date +%s) - start) / 60 )); log "status='$status' elapsed=${elapsed}m"
  if [ "$status" = "ALL_DONE" ]; then
    bash scripts/reuse_ig/lambda_run.sh "$IP" fetch; ok=1
    for tag in llama31_8b mistral7b_v03 gemma2_9b qwen25_14b olmo2_7b; do
      f="data/reuse_ig/items/downstream/reuse_$tag/logprobs_reuse.jsonl"
  ...
  if [ "$elapsed" -ge "$MAX_WAIT_MIN" ]; then bash scripts/reuse_ig/lambda_run.sh "$IP" fetch; terminate; log "DONE_TIMEOUT"; exit 2; fi
  sleep 120
done
