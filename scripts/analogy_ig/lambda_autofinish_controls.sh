...
set -u
IP="${1:?ip}"; INST="${2:?instance id}"; EXPECT=977; MAX_WAIT_MIN=120
...
while true; do
  ...
  if [ "$status" = "ALL_DONE" ]; then
    ...
    for run in logprob_llama31_8b_projdir logprob_mistral7b_v03_projdir logprob_gemma2_9b_projdir logprob_qwen25_14b_projdir logprob_olmo2_7b_projdir; do
      for c in format named; do
        f="data/analogy_ig/dataset/downstream/$run/downstream/${c}_control/logprobs_${c}.jsonl"
